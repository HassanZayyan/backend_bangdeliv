    ...
    <div class="panel-header">
        <div style="display:flex; align-items:center; gap:12px;">
            ...
            <div>
                ...
                <div style="font-size:12px; color:var(--text-muted); margin-top:2px;">Isi data lengkap restoran atau warung baru</div>
            </div>
        </div>
    ...
    <form action="{{ route('admin.restaurants.store') }}" method="POST" enctype="multipart/form-data" style="padding: 24px;">
        ...
        <div style="margin-bottom: 24px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:16px;">
                ...
                <div class="form-group" style="margin-bottom:0;">
                    ...
                    <select name="merchant_type" class="form-control" required>
                        @foreach($merchantTypes->options() as $value => $label)
                            <option value="{{ $value }}" @selected(old('merchant_type', 'restaurant') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" style="margin-bottom:0; grid-column: span 2;">
                    <label>Nomor Telepon <span style="font-size:12px; font-weight:400; color:var(--text-muted);">(opsional)</span></label>
            ...
            <div style="display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:16px;">
                ...
                <div style="grid-column: span 2; display:flex; gap:10px; align-items:center; flex-wrap:wrap; background:var(--bg-body); border:1px solid var(--border-color); border-radius:8px; padding:12px 16px;">
                    ...
                    <span style="font-size:12px; color:var(--text-muted);">
                        <i class='bx bx-info-circle'></i> Koordinat yang disalin dari Google Maps boleh memakai koma—sistem merapikannya otomatis.
                    </span>
                </div>
            </div>
        </div>
